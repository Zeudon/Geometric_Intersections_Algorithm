...
//The point structure created for each point which stores info such as type of the point and the index to identify the line segment
//Type of the point= left endpoint,right endpoint or intersection point
...
//The segment structure for each line segment which stores info such as the endpoints and the y coordinate of intersection with the sweep line
struct segment
{
    double x1;
    double y1;
 ...
 //The class for each node of the status data structure, implemented using AVL tree
class StatusNode
{
    public:
    segment line;
...
//The class for each node of the event queue, implemented using AVL tree
class Node
{
    public:
    point p;
...
//Function to find the y coordinate, given a segment and the x coordinate
//If line is vertical,then we consider y coordinate of left endpoint
//Else we use y=m*x + c
...
//function to return depth of a node in the tree
int depth(Node *t)
{
    if(t==NULL)
    {
...
//function to create a node for the event queue
Node* create(point p)
{
    Node* temp=new Node();
    temp->p=p;
...
// Left and Right rotate functions that helps in balancing the AVL tree for Event Queue data structure
...
//Function to insert a node containing the point into the tree
Node* insert(Node *t,point p)
{
    ...
    {
        //If the point is same,endpoint of 2 line segments could be same, then its an intersection point
        if(p.y==t->p.y)
        {
            //t->counter++;
        ...
        {
            //if y coordinate of current point is higher, we insert it in the left subtree
            if(p.y>t->p.y)
            {
                t->left = insert(t->left,p);
            }
    ...
    //computes depth of the node
    t->depth = max(depth(t->left),depth(t->right))+1;

    int balanced = check(t);
    //cout<<value<<" "<<balanced<<"\n";

    //Conditions for balancing the AVL Tree after insertion
...
//Function to delete a node from the AVL tree for Event Queue
Node* deleteNode(Node* root,point p)
{
    ...
    //traversing the tree to find the node to delete
    if(p.x<root->p.x)
    {
    ...
    else
    {
        if(p.y==root->p.y)
        {
            ...
            {
                //finding minimum node to the right subtree of the node to be deleted
...
//Function to find the immediate successor to the given node
...
//Function to find the immmediate predecessor to the given node
...
//Function to find depth of the given node in status data structure
...
//Function to compute the intersection points with the sweep line
...
int main()
{
    ofstream output_file("output_file3.txt");
