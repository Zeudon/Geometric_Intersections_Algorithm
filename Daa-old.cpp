...
double finder(segment s,double x)
{
    ...
    if(s.x1==s.x2)
    {
        //if line is vertical then intercept is left endpoint
        y_intercept=s.y1;
    }
    else
    {
...
{
    point p;
    //cases to check if both line are vertical or if one is vertical or none are
    ...
    {
        //find the slope and generate equations to find intersection point
        ...
        if((y<=ma1 && y>=mi1) && (y<=ma2 && y>=mi2))
        {
            ...
            p.ind1=s1.p1.ind1;
            p.ind2=s2.p1.ind1;
            //Insert the intersection point in queue
            root1=insert(root1,p);
        }
    }
    else if(s2.x1==s2.x2)
...
StatusNode* insert1(StatusNode *t,segment ls,double x,double y)
{
    ...
    double y1,y2;
    //balancing with respect to y intercept of root and given node
    if(balanced>1)
    {
...
StatusNode* deleteStatusNode1(StatusNode* root,segment ls, double x, double y)
{
    ...
    //balancing with respect to y intercept of the line and root with sweep line
...
int main()
{
    ...
    while(remaining>0)
    {
        //execute till all points are visited
        remaining-=1;
        Node* temp=minNode(root1);
        if(temp->p.left==1)
        {
            //point is a left endpoint
